...
// reads words from a file, expects them to be comma-separated on one line
//vector<string> readWordsFromFile(const string& filename) {
//    ifstream inputFile(filename);
//    if (!inputFile) {
//        cerr << "error opening file!" << endl;
...
//    vector<string> words;
//    string line;
//    if (getline(inputFile, line)) { // read the entire line
//        size_t pos = 0;
//        while ((pos = line.find(',')) != string::npos) {
//            words.push_back(line.substr(0, pos)); // extract word before comma
//            line.erase(0, pos + 1); // remove processed part
//        }
//        words.push_back(line); // add the last word after the last comma
//    }
//    inputFile.close();
//    return words;
//}
...
// example of input after being sorted (one word per line) (works best):
//bag
//bag
//ball
//ball
...
// example of input after being sorted (separated by commas) (not so confident in this):
//bag, bag, ball, ball, base, base, golf, tee, the, them